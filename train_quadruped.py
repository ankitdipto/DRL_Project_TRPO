...
if __name__ == "__main__":
    ...
    print(f"  Steps per epoch: {steps_per_epoch}")
    print(f"  Total steps per update: {steps_per_epoch * num_envs}")
    print(f"  Evaluation frequency: every {eval_freq} epochs")
    ...
    #     num_threads=min(4, num_envs),
    #     frame_skip=10,  # Skip frames for realtime video (500Hz / 10 = 50Hz = 50fps)
    # )
    vec_env = make_quadruped_env(num_envs=num_envs, model_path=model_path, use_rollout=False)
    ...
    policy = GaussianPolicy(obs_dim, act_dim, hidden_dim=hidden_dim)
    value_net = ValueNetwork(obs_dim, hidden_dim=hidden_dim)
    ...
    print(f"View logs with: tensorboard --logdir=runs/trpo_quadruped")
